...
def multi_source_lookup(event: dict, api_key: str, run_type: str) -> dict:
    """Run all available sources and merge results. Perplexity fills actual game times."""
    ...
    # 8. Perplexity — actual game times (start, end, notes, details)
    if api_key:
        ...
        combined["sources_used"].append("Perplexity")
...
# ─────────────────────────────────────────────
# PERPLEXITY GAME TIMES LOOKUP
# ─────────────────────────────────────────────
def google_game_times_lookup(event: dict, api_key: str) -> dict:
    """Use Perplexity sonar-pro to find actual game start time, end time, and notes.
    Queries: 'What time did [event] on [date] start and end in ET?'
    Returns: actual_start, actual_end, notes, details (raw answer text).
    """
    ...
    # Build a natural-language query matching what Perplexity handles best
    q = f"What time did the {name} game on {date_str} start and end in ET (Eastern Time)? Give exact start time and end time in ET."
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": (
                            "You are a sports data assistant. When asked about game times, "
                            "search ESPN, WNBA.com, NBA.com, MLB.com, or sports reference sites "
                            "for the actual start and end times. Always give times in ET (Eastern Time). "
                            "Be concise: state the start time and end time only, in the format "
                            "'Start: H:MM AM/PM ET. End: H:MM AM/PM ET.' If the exact end time is "
    ...
    # Store raw answer as DETAILS
    result["details"] = answer

    # ── Extract times from the answer text ──────────────────────────────────
    # Patterns: "9:00 PM ET", "9:05 PM ET", "11:16 PM ET", "11:00 p.m. ET"
    TIME_ET = re.compile(
        r'(\d{1,2}:\d{2}\s*(?:AM|PM|a\.m\.|p\.m\.)\s*(?:ET|EST|EDT|Eastern(?:\s+Time)?))',
        re.IGNORECASE
    )
    ...
    def _clean_time(t: str) -> str:
        ...
        t = re.sub(r'\bEST\b|\bEDT\b', 'ET', t, flags=re.IGNORECASE)
        return t.strip()

    # Try explicit "Start: ... End: ..." labels first
    start_label = re.search(
        r'(?:Start(?:ed)?|Tip.?off|Kicked off|First pitch)[:\s]+' +
        r'(\d{1,2}:\d{2}\s*(?:AM|PM|a\.m\.|p\.m\.)\s*(?:ET|EST|EDT|Eastern(?:\s+Time)?)?)',
        answer, re.IGNORECASE
    )
    end_label = re.search(
        r'(?:End(?:ed)?|Final|Concluded|Finished)[:\s]+' +
        r'(\d{1,2}:\d{2}\s*(?:AM|PM|a\.m\.|p\.m\.)\s*(?:ET|EST|EDT|Eastern(?:\s+Time)?)?)',
        answer, re.IGNORECASE
    )

    if start_label:
        result["actual_start"] = _clean_time(start_label.group(1)) + (" ET" if not re.search(r'ET|EST|EDT|Eastern', start_label.group(1), re.I) else "")
    if end_label:
        result["actual_end"] = _clean_time(end_label.group(1)) + (" ET" if not re.search(r'ET|EST|EDT|Eastern', end_label.group(1), re.I) else "")

    # Fallback: collect all ET times in order → first = start, last = end
    if not result["actual_start"] or not result["actual_end"]:
        et_times = TIME_ET.findall(answer)
        et_cleaned = [_clean_time(t) for t in et_times]
        # Deduplicate while preserving order
        seen = []
        for t in et_cleaned:
            norm = re.sub(r'\s+', ' ', t).upper()
            if norm not in [re.sub(r'\s+', ' ', x).upper() for x in seen]:
                seen.append(t)
        if seen and not result["actual_start"]:
            result["actual_start"] = seen[0]
        if len(seen) > 1 and not result["actual_end"]:
            result["actual_end"] = seen[-1]
        elif len(seen) == 1 and not result["actual_end"]:
            # Only one time found — could be start only; leave end blank
            pass

    # ── Extract notes (overtime, delay, etc.) ───────────────────────────────
    NOTES_PATS = [
    ...
    for pat, label in NOTES_PATS:
        if pat.search(answer) and label not in notes:
            notes.append(label)
    result["notes"] = "; ".join(notes)
    # ── Second Perplexity query: dedicated overtime / delay / postpone search ──
    # Only fires if the first answer didn't already mention these events
    NEEDS_NOTES_CHECK = not notes and api_key
    if NEEDS_NOTES_CHECK:
        q_notes = (
            f"Did the {name} game on {date_str} go into overtime, have any weather delays, "
            f"rain delays, or get postponed or canceled? Answer yes or no and give details."
        )
        try:
            resp2 = requests.post(
                "https://api.perplexity.ai/chat/completions",
                    ...
                    "messages": [
                        {
                            "role": "system",
                            "content": (
                                "You are a sports data assistant. Answer only whether the game "
                                "went to overtime, had weather/rain delays, was postponed, or was "
                                "canceled. Be concise. If none of these apply, say 'No notable delays or overtime.'"
                            )
                        },
                        {"role": "user", "content": q_notes}
                    ],
                    "max_tokens": 150,
                    "temperature": 0.0,
                },
            ...
            notes_answer = resp2.json()["choices"][0]["message"]["content"].strip()
            # Scan the notes answer for event keywords
            for pat, label in NOTES_PATS:
                if pat.search(notes_answer) and label not in notes:
                    notes.append(label)
            result["notes"] = "; ".join(notes)
            # Append notes answer to DETAILS if it found something
            if notes:
                result["details"] = result["details"] + " | Notes: " + notes_answer
        except Exception:
            pass  # notes query failure is non-fatal


    # ── Third Perplexity query: what networks/channels broadcast the game ────
    if api_key:
        q_broadcast = (
            f"What TV channel or network broadcast the {name} game on {date_str}? "
            f"List all national networks, local channels, and streaming platforms that aired it."
        )
        try:
            resp3 = requests.post(
                "https://api.perplexity.ai/chat/completions",
                    ...
                    "messages": [
                        {
                            "role": "system",
                            "content": (
                                "You are a sports broadcast data assistant. List every TV channel, "
                                "cable network, and streaming platform that aired this game. "
                                "Be concise and specific (e.g., ESPN, TNT, Peacock, local RSN names)."
                            )
                        },
                        {"role": "user", "content": q_broadcast}
                    ],
                    "max_tokens": 200,
                    "temperature": 0.0,
                },
            ...
            broadcast_answer = resp3.json()["choices"][0]["message"]["content"].strip()
            if broadcast_answer:
                result["details"] = result["details"] + " | Broadcast: " + broadcast_answer
        except Exception:
            pass  # broadcast query failure is non-fatal
    return result

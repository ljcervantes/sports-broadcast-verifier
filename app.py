...
def google_game_times_lookup(event: dict, api_key: str) -> dict:
    ...
    """
    result = {"actual_start": "", "actual_end": "", "notes": ""}
    if not api_key:
        return result
    ...
    def _extract_time_from_data(data: dict, prefer_end: bool = False) -> tuple[str, str, list[str]]:
        ...
        def _add_time(t_raw: str):
            t = t_raw.strip().upper()
            if not re.search(r'\bET\b|\bCT\b|\bPT\b|\bMT\b|\bEST\b|\bEDT\b', t):
                t += " ET"
        ...
        for organic in data.get("organic_results", []):
            ...
            # All other times in snippet
            for t in TIME_PAT.findall(snippet):
                _add_time(t)

        # Derive start and end from collected times
        # Prefer anchor-matched times; fall back to first/last of all times
        valid_times = [t for t in all_times if re.search(r'\d{1,2}:\d{2}', t)]
        if start_anchors:
            start = start_anchors[0]
        elif valid_times and not prefer_end:
            start = valid_times[0]
        else:
            start = ""
        if end_anchors:
        ...
        # If only one time found and prefer_end, treat it as end
        if prefer_end and valid_times and not end:
            end = valid_times[0]
            start = ""
...
def run_verification(events: list[dict], run_type: str, api_key: str, run_name: str) -> pd.DataFrame:
    ...
    for col_def in [("video_duration","TEXT"), ("sources_used","TEXT"), ("notes","TEXT")]:
        try:
    ...
    for i, event in enumerate(events):
        ...
        c.execute("""INSERT INTO verification_results
            (run_id,league,event_name,event_date,sched_start,sched_channel,
             ...
             streaming,verdict,notes,search_url,sources_used)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
    ...
    with tab_dash:
        ...
        if "results_df" in st.session_state:
            ...
            export_cols = ["league","event_name","event_date","sched_start","sched_channel",
                           ...
                           "local_home","local_away","streaming","notes","verdict","sources_used"]
            ...
            col_labels = {
                ...
                "notes":"Notes","verdict":"Verdict","sources_used":"Sources Used",
            }

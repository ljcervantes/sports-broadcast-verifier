...
_TIME_PAT = re.compile(
    ...
    r'(?:\s*:?\d{2})?'
    r'(?:\s*(?:a\.?m\.?|p\.?m\.?|[AaPp][Mm]))?'
    r'(?:\s*(?:ET|EST|EDT|Eastern(?:\s+(?:Standard|Daylight)?\s*Time)?))?',
    re.IGNORECASE
)
_START_CTX = re.compile(
    r'\b(?:start(?:ed|s|ing)?|tip(?:ped)?(?:\s*-?\s*off)?|kick(?:ed)?(?:\s*off)?|'
    r'begin(?:ning|s)?|began|open(?:ed|ing)?|launch(?:ed)?|'
    r'first\s+(?:pitch|quarter|half|period|tip)|puck\s+drop|'
    r'game\s+(?:time|start|began|kicked|tipped)|'
    r'tip\s*-?\s*off\s+(?:was|is|at)|'
    r'scheduled\s+(?:for|at|start|tip)|listed\s+(?:at|for)|'
    r'air(?:ed|s)?\s+at|pregame|pre-?game)\b',
    re.IGNORECASE
)
_END_CTX = re.compile(
    r'\b(?:end(?:ed|s|ing)?|finish(?:ed|es)?|conclud(?:ed|es)?|complet(?:ed|es)?|'
    r'wrap(?:ped)?(?:\s*up)?|final(?:\s+(?:whistle|buzzer|out|score|result))?|'
    r'last(?:ed)?|ran?\s+(?:until|to|through)|went\s+(?:until|to)|'
    r'over\s+(?:at|by|around)|done\s+(?:at|by)|'
    r'clock(?:ed)?\s+out|buzzer|postgame|post-?game|'
    r'game\s+(?:end|finish|conclud|complet|wrap)|'
    r'end\s+time|final\s+(?:time|score\s+(?:was\s+)?(?:posted|reported)\s+at))\b',
    re.IGNORECASE
)
_FORCE_END = re.compile(
    r'\b(?:postgame|post-?game|final\s+(?:score|whistle|buzzer)|game\s+(?:ended|concluded|finished|wrapped))\b',
    re.IGNORECASE
)
_FORCE_START = re.compile(
    r'\b(?:pregame|pre-?game|tip-?off\s+(?:was|is)|game\s+time\s+(?:is|was)|opening\s+tip)\b',
    re.IGNORECASE
)

def _get_sentence(text, m):
    sent_start = max(0, max(text.rfind('.', 0, m.start()), text.rfind('\n', 0, m.start())) + 1)
    sent_end_dot = text.find('.', m.end())
    sent_end_nl  = text.find('\n', m.end())
    sent_end = len(text)
    if sent_end_dot != -1: sent_end = min(sent_end, sent_end_dot)
    if sent_end_nl  != -1: sent_end = min(sent_end, sent_end_nl)
    return text[sent_start:sent_end], sent_start

def _score_ctx(text, m):
    sentence, sent_start = _get_sentence(text, m)
    before = sentence[:m.start() - sent_start].lower()
    after  = sentence[m.end() - sent_start:].lower()
    s = len(_START_CTX.findall(before)) * 3 + len(_START_CTX.findall(after))
    e = len(_END_CTX.findall(before)) * 3 + len(_END_CTX.findall(after))
    if _FORCE_END.search(sentence):
        e = max(e, s + 1)
    if _FORCE_START.search(sentence) and not _FORCE_END.search(sentence):
        s = max(s, e + 1)
    return s, e, sent_start

def _norm_time(raw):
    t = raw.strip()
    t = re.sub(r'a\.m\.?', 'AM', t, flags=re.IGNORECASE)
    t = re.sub(r'p\.m\.?', 'PM', t, flags=re.IGNORECASE)
    t = re.sub(r'\b(?:Eastern(?:\s+(?:Standard|Daylight)?\s*Time)?)\b', 'ET', t, flags=re.IGNORECASE)
    t = re.sub(r'\b(?:EST|EDT)\b', 'ET', t, flags=re.IGNORECASE)
    t = re.sub(r'\b(?:approximately|roughly|around|about)\s+', '', t, flags=re.IGNORECASE)
    t = t.strip()
    if not re.search(r'\bET\b', t, re.IGNORECASE):
        t += ' ET'
    return t.strip()

def _smart_extract_times(text):
    cands = []
    for m in _TIME_PAT.finditer(text):
        if re.match(r'^\d:\d{2}:\d{2}$', m.group(0).strip()):
            continue
        s, e, sent_start = _score_ctx(text, m)
        cands.append({'norm': _norm_time(m.group(0)), 'pos': m.start(), 's': s, 'e': e, 'sent': sent_start})
    if not cands:
        return '', ''
    if len(cands) == 1:
        return cands[0]['norm'], ''
    ...
    start_pool, end_pool = [], []
    for sent_start, group in by_sent.items():
        group_sorted = sorted(group, key=lambda c: c['pos'])
        if len(group_sorted) >= 2:
            start_pool.append(group_sorted[0])
            end_pool.append(group_sorted[-1])
        else:
            c = group_sorted[0]
            if c['s'] > c['e']:
                start_pool.append(c)
    ...
    start_time = ''
    if start_pool:
        best = max(start_pool, key=lambda c: (c['s'], -c['pos']))
        start_time = best['norm']
    end_time = ''
    if end_pool:
        best = max(end_pool, key=lambda c: (c['e'], c['pos']))
        end_time = best['norm']
    if not end_time:
        all_neutral = [c for c in cands if c['s'] == c['e']]
        if len(all_neutral) > 1:
            latest = max(all_neutral, key=lambda c: c['pos'])
            if latest['norm'] != start_time:
                end_time = latest['norm']
    if start_time == end_time:
        end_time = ''
    return start_time, end_time


def _smart_extract_duration(text):
    m = re.search(r'^Duration:\s*(.+?)$', text, re.IGNORECASE | re.MULTILINE)
    if m:
        val = m.group(1).strip()
        if not re.search(r'not available|unknown|n/a', val, re.IGNORECASE):
            return val
    m_hms = re.search(r'\b(\d{1,2}:\d{2}:\d{2})\b', text)
    if m_hms:
        pre = text[max(0, m_hms.start()-10):m_hms.start()].lower()
        if not re.search(r'\bat\s*$|\baround\s*$|\bby\s*$', pre):
            return m_hms.group(1)
    m_dur = re.search(
    ...
    if m_range:
        return f"{m_range.group(1)} hours {m_range.group(2)} minutes"
    m_prose = re.search(
...
_LOCAL_KWORDS = ["local", "market", "regional", "rsn"]

def _clean_channel_item(item, channels):
    label_m = re.match(
        r'^(?:national|local|home|away|streaming|stream|regional|rsn|cable|ott|league)\s*(?:[\w/\s]+)?:\s*(.+)',
        item, re.IGNORECASE
    )
    if label_m:
    ...
    if _pm:
        _c = _pm.group(1)
        if re.match(
            r'^(?:national|local|home|away|primary|secondary|alternate|regional|streaming|cable|ott|league|'
            r'chicago|portland|seattle|boston|new york|los angeles|san francisco|miami|dallas|houston|'
            r'phoenix|denver|atlanta|washington|philadelphia|minneapolis|detroit|cleveland|orlando|'
            r'sacramento|oklahoma city|salt lake|new orleans|memphis|milwaukee|charlotte|indiana|'
            r'toronto|golden state|san antonio|brooklyn)\s*(?:local|regional|market|feed|area|based)?\s*$',
            _c, re.IGNORECASE
        ):
...
def _smart_extract_broadcast(text):
    bcast_raw = ""
    m_bcast = re.search(r'^Broadcast:\s*(.+?)$', text, re.IGNORECASE | re.MULTILINE)
    if m_bcast:
    ...
    if not bcast_raw:
        ...
        if m_sec:
            ...
            block_end_m = re.search(r'\n(?:#{1,3}\s|\-{3,})', text[block_start:])
            block = text[block_start: block_start + (block_end_m.start() if block_end_m else 1500)]
            channels = []
            for line in block.split('\n'):
                ...
                if appears_m:
                    ...
                    if ch and ch not in channels:
                        channels.append(ch)
                    continue
                _clean_channel_item(item, channels)
            if channels:
    ...
    if not bcast_raw:
        ...
        bcast_raw = m_p.group(1).strip() if m_p else ""
    if not bcast_raw:
        m_p = re.search(
    ...
    if not bcast_raw:
        ...
        if m_p:
            lines = re.findall(r'[-*•]\s*(.+)', m_p.group(1))
...
def _classify_broadcast(bcast_raw, home_team, away_team):
    if not bcast_raw:
        return '', '', '', '', ''
    parts = re.split(r'[;,]|\band\b', bcast_raw, flags=re.IGNORECASE)
    home_team_l = (home_team or '').lower()
    away_team_l = (away_team or '').lower()
    ...
    for raw_part in parts:
        ...
        if not p:
            continue
        pl = p.lower()
        if any(kw in pl for kw in _STREAMING_KWORDS):
            streamers.append(p)
            continue
        if any(kw in pl for kw in _LOCAL_KWORDS):
            if home_team_l and any(tok in pl for tok in home_team_l.split() if len(tok) > 3):
                locals_.append(('home', p))
            elif away_team_l and any(tok in pl for tok in away_team_l.split() if len(tok) > 3):
                locals_.append(('away', p))
            else:
                locals_.append(('home', p))
            continue
        p_norm = re.sub(r'\(.*?\)', '', pl).strip()
        if any(p_norm == net or net in p_norm for net in _NATIONAL_NETS):
            nationals.append(p)
            continue
        if re.match(r'^[A-Z]{2,5}(\d)?(-[A-Z]+)?$', p.strip()):
            ...
            continue
        nationals.append(p)
    national_channel = '; '.join(nationals) if nationals else ''
    home_locals = [ch for role, ch in locals_ if role == 'home']
    away_locals = [ch for role, ch in locals_ if role == 'away']
    local_home  = '; '.join(home_locals) if home_locals else ''
    local_away  = '; '.join(away_locals) if away_locals else ''
    streaming   = '; '.join(streamers) if streamers else ''
    all_channels = nationals + [ch for _, ch in locals_] + streamers
    broadcast_channels = ', '.join(all_channels) if all_channels else bcast_raw
    return national_channel, local_home, local_away, streaming, broadcast_channels

# ─────────────────────────────────────────────
# END SMART EXTRACTION HELPERS
# ─────────────────────────────────────────────
...
def _parse_zoomph_df(df: pd.DataFrame) -> list[dict]:
    ...
    for (date, home_l, away_l), rows in groups.items():
        ...
        # If no real broadcast channel found, fall back to any network value (including WCIUDT)
        # so games that only aired on the streaming platform still show a channel
        if not sched_channel:
            fallback_nets = []
            for r in rows:
                net_raw    = safe(r, network_col)
                net_parent = safe(r, net_parent_col)
                ch = net_raw or net_parent
                if ch and ch not in fallback_nets:
                    fallback_nets.append(ch)
            sched_channel = ", ".join(fallback_nets) if fallback_nets else ""
...
def google_game_times_lookup(event: dict, api_key: str) -> dict:
                ...
                "messages": [
                    {
                        ...
                        "content": (
                            ...
                            "(3) whether there were any weather/rain delays, postponements, or cancellations. "
    ...
    # Post-process: strip markdown structure so Details is always plain paragraphs
    # 1. Remove ### / ## / # section headers entirely (the whole line)
    clean_answer = re.sub(r'^#{1,3}\s+.+$', '', clean_answer, flags=re.MULTILINE)
    # 2. Convert bullet/list lines (- item, * item, • item) into plain sentences
    #    by stripping the leading marker and joining with the previous paragraph
    clean_answer = re.sub(r'^\s*[-*•]\s+', '', clean_answer, flags=re.MULTILINE)
    # 3. Remove horizontal dividers (--- or ***)
    clean_answer = re.sub(r'^\s*[-*]{3,}\s*$', '', clean_answer, flags=re.MULTILINE)
    ...
    # Store the cleaned Perplexity answer verbatim as Details
    result["details"] = clean_answer

    # Store citation URLs as sources
    result["perplexity_sources"] = citations

    # ── Extract times (smart context-aware extractor) ─────────────────────────
    result["actual_start"], result["actual_end"] = _smart_extract_times(clean_answer)

    # ── Extract Duration ─────────────────────────────────────────────────────
    result["video_duration"] = _smart_extract_duration(clean_answer)
    ...
    bcast_raw = _smart_extract_broadcast(clean_answer)
    if bcast_raw:
        home_team = event.get("home_team", "")
        away_team = event.get("away_team", "")
        nat, lh, la, st, bc = _classify_broadcast(bcast_raw, home_team, away_team)
        result["national_channel"]   = nat
        result["local_home"]         = lh
        result["local_away"]         = la
        result["streaming"]          = st
        result["broadcast_channels"] = bc

    # ── Notes column is always blank ─────────────────────────────────────────
    result["notes"] = ""

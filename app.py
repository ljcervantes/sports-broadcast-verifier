...
def multi_source_lookup(event: dict, api_key: str, run_type: str) -> dict:
    ...
    if api_key:
        ...
        if ggt.get("video_duration"):
            combined["video_duration"] = ggt["video_duration"]
        if ggt.get("notes"):
            combined["notes"] = ggt["notes"]
        if ggt.get("details"):
            combined["details"] = ggt["details"]
        # Replace sources_used with the actual Perplexity citation URLs
        plex_srcs = ggt.get("perplexity_sources", [])
        if plex_srcs:
            combined["sources_used"] = plex_srcs  # list of URLs
        else:
...
# ─────────────────────────────────────────────
# PERPLEXITY GAME TIMES LOOKUP  (single query)
# ─────────────────────────────────────────────
def google_game_times_lookup(event: dict, api_key: str) -> dict:
    """Single Perplexity sonar-pro query per game.
    Asks for start time, end time, duration, broadcast channels, and any delays/OT.
    Returns: actual_start, actual_end, video_duration, notes, details (raw answer), sources (citation URLs).
    """
    result = {
        "actual_start": "", "actual_end": "", "video_duration": "",
    ...
    q = (
        f"What time did the {name} game on {date_str} start and end in Eastern Standard Time? "
        f"What broadcast channel(s) did it air on? "
        f"What was the total video duration? "
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": (
                            "You are a sports data assistant. Always give times in ET (Eastern Time). "
                            "Format your response exactly like this:\n"
                            "Start: H:MM AM/PM ET\n"
                            "End: H:MM AM/PM ET (or \'Not available\' if unknown)\n"
                            "Duration: X hours Y minutes (or \'Not available\' if unknown)\n"
                            "Broadcast: [list all national networks, local channels, and streaming platforms]\n"
                            "Notes: [overtime, delays, postponements — or \'None\' if none]\n"
                            "Then provide a brief 1-2 sentence summary of the game result."
                        )
                    },
                    {"role": "user", "content": q}
                ],
    ...
    # Only flag real events — ignore "None", "No", "N/A"
    if notes_text and not re.match(r'^(none|no|n/a|not applicable)', notes_text, re.IGNORECASE):
        NOTES_PATS = [
        ...
        for pat, label in NOTES_PATS:
            if pat.search(notes_text) and label not in found:
                found.append(label)
        result["notes"] = "; ".join(found) if found else notes_text
...
def render_results_table(df: pd.DataFrame):
    ...
    for _, row in df.iterrows():
        ...
        search_link = ""  # Google search link removed — Perplexity sources used instead
        ...
        # Show first 200 chars with ellipsis; full text in tooltip
        details_disp  = (details_raw[:200] + "…") if len(details_raw) > 200 else (details_raw or "—")
        details_title = details_raw.replace('"', '&quot;').replace("'", "&#39;")
        ...
        <tr style="background:{bg}; border-bottom:1px solid #30363d;">
            ...
            <td style="padding:6px 10px; color:#c9d1d9; font-size:11px; max-width:400px; min-width:260px; white-space:normal; word-wrap:break-word; line-height:1.4;" title="{details_title}">{details_disp}</td>
...
def render_sidebar():
    ...
    conn = get_db()
    c = conn.cursor()
    # Load from perplexity_key; fall back to serpapi_key for users upgrading from old version
    c.execute("SELECT value FROM settings WHERE key='perplexity_key'")
    row = c.fetchone()
    if row:
        saved_key = row["value"]
    else:
        # Migrate from old serpapi_key name if present
        c.execute("SELECT value FROM settings WHERE key='serpapi_key'")
        old_row = c.fetchone()
        saved_key = old_row["value"] if old_row else ""
    conn.close()

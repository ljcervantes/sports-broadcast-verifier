...
def google_game_times_lookup(event: dict, api_key: str) -> dict:
    ...
    # End-time anchor phrases
    END_PATS = [
        re.compile(r'(?:ended?|final|concluded?|finished?|wrapped?|over)\s+(?:at\s+)?(\d{1,2}:\d{2}\s*(?:AM|PM)\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT)?)', re.IGNORECASE),
        re.compile(r'(\d{1,2}:\d{2}\s*(?:AM|PM)\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT)?)\s*(?:final|end|ended)', re.IGNORECASE),
    ]
    ...
    def _extract_time_from_data(data: dict, prefer_end: bool = False) -> tuple[str, str, list[str]]:
            ...
            if status_raw:
                for pat, label in NOTES_PATTERNS:
                    if pat.search(status_raw) and label not in notes:
                        notes.append(label)
                # Also keep raw status if it is short and informative
        ...
        if isinstance(ab, dict):
            for field in ["answer", "result", "snippet"]:
                val = ab.get(field, "")
                if val:
                    times = TIME_PAT.findall(val)
                    if times:
                        t = times[0].strip().upper()
                        if not t.endswith("ET") and not t.endswith("PM") and not t.endswith("AM"):
                            t += " ET"
                        if prefer_end and not end:
                            end = t
                        elif not prefer_end and not start:
                            start = t
                    for pat, label in NOTES_PATTERNS:
        ...
        for organic in data.get("organic_results", []):
            ...
            for pat, label in NOTES_PATTERNS:
                if pat.search(snippet) and label not in notes:
                    notes.append(label)
            # End time from anchor phrases
            if not end:
                for ep in END_PATS:
                    m = ep.search(snippet)
                    if m:
                        t = m.group(1).strip().upper()
                        if not re.search(r'\bET\b|\bCT\b|\bPT\b|\bMT\b', t):
                            t += " ET"
                        end = t
                        break
            # Start time from snippet (only if no start found yet)
            if not start and not prefer_end:
                times = TIME_PAT.findall(snippet)
                if times:
                    t = times[0].strip().upper()
                    if not re.search(r'\bET\b|\bCT\b|\bPT\b|\bMT\b', t):
                        t += " ET"
    ...
    # Query 1: game start time
    q_start = f"what time did the {league} {name} game start {date_str}"
    data_start = _query_google(q_start)
    s1, e1, notes1 = _extract_time_from_data(data_start, prefer_end=False)
    if s1:
        result["actual_start"] = s1
    if e1:
        result["actual_end"] = e1

    # Query 2: game end time (only if end not already found)
    if not result["actual_end"]:
        q_end = f"what time did the {league} {name} game end {date_str}"
        data_end = _query_google(q_end)
        _, e2, notes2 = _extract_time_from_data(data_end, prefer_end=True)
        if e2:
            result["actual_end"] = e2
        notes1 = list(dict.fromkeys(notes1 + notes2))  # deduplicate, preserve order

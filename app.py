...
def detect_columns(df: pd.DataFrame) -> dict:
    ...
    # League
    for k in ["league","sport","competition","league_name"]:
        if k in cols: mapping["league"] = cols[k]; break

    # Date
    for k in ["date","game_date","event_date","air_date","gamedate"]:
        if k in cols: mapping["date"] = cols[k]; break

    # Home team / event name
    for k in ["home","home_team","hometeam","event","event_name","title","program","name"]:
        if k in cols: mapping["home"] = cols[k]; break

    # Away team
    for k in ["away","away_team","awayteam","visitor","visiting_team"]:
        if k in cols: mapping["away"] = cols[k]; break

    # Channel
    for k in ["channel","network","broadcast","broadcast_network","station","tv","air_channel","scheduled_channel"]:
...
def main():
    ...
    with tab_dash:
        ...
        if "schedule_df" in st.session_state:
            ...
            preview_df = pd.DataFrame(events)[["league","event_name","event_date","sched_start","sched_channel"]]
            preview_df.columns = ["League","Event","Date","Sched. Start","Sched. Channel"]

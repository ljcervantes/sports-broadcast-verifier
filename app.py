...
def google_game_times_lookup(event: dict, api_key: str) -> dict:
    ...
    if m_bcast:
        ...
        # Fallback 1: Section header + bullet list
        # Matches "### Broadcast channels\n- National: ION\n- Chicago local: CW26..."
        # Matches "Broadcast / stream listings\n- CW Seattle\n- CW26..."
        # The optional non-bullet prose line handles "The game had both national and local TV coverage:"
        ...
        if m_bcast_prose:
            bullet_block = m_bcast_prose.group(1)
            lines = re.findall(r'[-*•]\s*(.+)', bullet_block)
            cleaned = []
            for bl in lines:
                bl = bl.strip()
                # Strip sub-category labels like "National: ", "Chicago local: ", "Streaming: "
                bl = re.sub(r'^[\w\s]+:\s+', '', bl)
                if bl:
                    cleaned.append(bl)
            bcast_raw = ", ".join(cleaned)
        if not bcast_raw:
            # Fallback 2: Prose "aired on X, Y, and Z" — stop before non-channel text
            m_bcast_prose = re.search(
        ...
        if not bcast_raw:
            # Fallback 3: "available on / listed on / airing on" + inline list
            m_bcast_prose = re.search(
            ...
            bcast_raw = m_bcast_prose.group(1).strip() if m_bcast_prose else ""
        if not bcast_raw:
            # Fallback 4: "aired on / available on" + colon + bullet list
            m_bcast_prose = re.search(
    ...
    if scan_text:
        ...
        NEGATION_RE = re.compile(
            ...
            r'non-overtime|non-ot\b|non.overtime)\b',
            re.IGNORECASE
        )
        found = []

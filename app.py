...
def google_game_times_lookup(event: dict, api_key: str) -> dict:
    ...
    END_PATS = [
        re.compile(r'ended?\s+(?:at\s+)?(?:approximately\s+)?(\d{1,2}:\d{2}\s*(?:AM|PM)\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT)?)', re.IGNORECASE),
        re.compile(r'(?:final|concluded?|finished?|wrapped?|over|completed?)\s+(?:at\s+)?(\d{1,2}:\d{2}\s*(?:AM|PM)\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT)?)', re.IGNORECASE),
        re.compile(r'(\d{1,2}:\d{2}\s*(?:AM|PM)\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT)?)\s*(?:final|end|ended|ET\)?)', re.IGNORECASE),
        re.compile(r'end\s+time[:\s]+(?:~)?(\d{1,2}:\d{2}\s*(?:AM|PM)\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT)?)', re.IGNORECASE),
    ]
    # Start-time anchor phrases
    START_PATS = [
        re.compile(r'started?\s+(?:at\s+)?(\d{1,2}:\d{2}\s*(?:AM|PM)\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT)?)', re.IGNORECASE),
        re.compile(r'start\s+time[:\s]+(\d{1,2}:\d{2}\s*(?:AM|PM)\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT)?)', re.IGNORECASE),
        re.compile(r'tip-?off\s+(?:at\s+)?(\d{1,2}:\d{2}\s*(?:AM|PM)\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT)?)', re.IGNORECASE),
        re.compile(r'first\s+pitch\s+(?:at\s+)?(\d{1,2}:\d{2}\s*(?:AM|PM)\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT)?)', re.IGNORECASE),
        re.compile(r'kickoff\s+(?:at\s+)?(\d{1,2}:\d{2}\s*(?:AM|PM)\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT)?)', re.IGNORECASE),
    ]
    ...
    def _extract_time_from_data(data: dict, prefer_end: bool = False) -> tuple[str, str, list[str]]:
        ...
        if isinstance(ab, dict):
            for field in ["answer", "result", "snippet", "description"]:
                ...
                ET_PAREN_PAT = re.compile(
                    r'\((?:[^)]*?/\s*)?(\d{1,2}:\d{2}\s*(?:AM|PM|a\.m\.|p\.m\.)\s*ET)\)',
                    re.IGNORECASE
                )
                # Find all ET-in-parens values in the text
                et_parens = ET_PAREN_PAT.findall(val)
                for ep_val in et_parens:
                    _add_time(ep_val)  # _add_time normalizes and deduplicates

                # Try anchor patterns on answer box text
        ...
        # Prefer the full answer_box text; fall back to first organic snippet
        raw_detail = ""
        if isinstance(ab, dict):
            for field in ["answer", "result", "snippet", "description"]:
                val = ab.get(field, "")
                if val and len(val) > 20:
                    ...
                    break
        if not raw_detail:
            for organic in data.get("organic_results", [])[:2]:
                snip = organic.get("snippet", "")
    ...
    s1, e1, notes1 = _extract_time_from_data(data, prefer_end=False)
    if s1:
    ...
    if not result["actual_end"] and result["actual_start"]:
        ...
        _, e2, notes2 = _extract_time_from_data(data_end, prefer_end=True)
        if e2:

...
def google_game_times_lookup(event: dict, api_key: str) -> dict:
    ...
    if scan_text:
        ...
        NEGATION_RE = re.compile(
            ...
            r'no sign of|no reports? of|no overtime|no delay|no weather)\b',
            re.IGNORECASE
        )
        found = []
        for pat, label in NOTES_PATS:
            for m in pat.finditer(scan_text):
                ...
                start = scan_text.rfind('.', 0, m.start())
                end = scan_text.find('.', m.end())
                sentence = scan_text[start+1 : end if end != -1 else len(scan_text)].strip()
                ...
                if re.match(r'^[-*•]\s*\w+\s*$', sentence):
                    continue
                # Skip sentences that are just listing topics (very short, no verb)
                if len(sentence.split()) <= 4 and not re.search(r'\b(went|occurred|happened|resulted|required|took)\b', sentence, re.IGNORECASE):
                    continue

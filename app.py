...
def multi_source_lookup(event: dict, api_key: str, run_type: str) -> dict:
    ...
    # 9. Perplexity Game Times — dedicated start/end/notes queries, highest priority for times
    # Load Perplexity key from DB (separate from SerpApi key)
    _pplx_conn = get_db()
    _pplx_row = _pplx_conn.execute("SELECT value FROM settings WHERE key='perplexity_key'").fetchone()
    _pplx_conn.close()
    pplx_key = _pplx_row["value"] if _pplx_row else ""
    if pplx_key:
        ggt = google_game_times_lookup(event, pplx_key)
        ...
        combined["sources_used"].append("Perplexity Times")
...
def render_sidebar():
    ...
    api_key = st.sidebar.text_input("SerpApi Key (broadcast lookup)", value=saved_key, type="password",
    ...
    # Perplexity API key for actual game times
    c2 = get_db().cursor()
    c2.execute("SELECT value FROM settings WHERE key='perplexity_key'")
    row2 = c2.fetchone()
    saved_pplx_key = row2["value"] if row2 else ""
    pplx_key = st.sidebar.text_input("Perplexity API Key (game times)", value=saved_pplx_key, type="password",
                                      help="Get a key at perplexity.ai/settings/api — used for actual start/end times")
    if pplx_key != saved_pplx_key:
        conn = get_db()
        conn.execute("INSERT OR REPLACE INTO settings (key,value) VALUES ('perplexity_key',?)", (pplx_key,))
        conn.commit()
        conn.close()
...
def main():
    ...
    with tab_dash:
        ...
        if "schedule_df" in st.session_state:
            ...
            if not api_key:
                st.warning("⚠️ No SerpApi key set — broadcast channel lookup will use local station matching only. Add your SerpApi key in the sidebar. For actual game times, add your Perplexity API key.")

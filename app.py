...
def google_game_times_lookup(event: dict, api_key: str) -> dict:
    ...
    q = (
        ...
        f"What time did the {matchup} game on {date_str} start and end in EST? "
        f"What broadcast channel(s) did it actually air on? "
        f"What was the video duration? "
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": (
                            "You are a sports broadcast data assistant. Always give times in ET (Eastern Time). "
                            "Format your response EXACTLY like this with no extra text before the labels:\n"
                            "Start: H:MM AM/PM ET\n"
                            "End: H:MM AM/PM ET\n"
                            "Duration: X hours Y minutes\n"
                            "Broadcast: [comma-separated list of ALL channels and streaming platforms]\n"
                            "Notes: [overtime/delays/postponements, or None]\n"
                            "Then 1-2 sentences summarizing the game result.\n"
                            "If a value is unknown, write Not available."
                        )
                    },
                    {"role": "user", "content": q}
                ],
    ...
    if not result["actual_start"]:
        m_start_prose = re.search(
            r'(?:started?|tipped?\s*off|kicked?\s*off|began?|aired?)\s+at\s+'
    ...
    if not result["actual_end"]:
        m_end_prose = re.search(
            r'(?:ended?|concluded?|finished?|wrapped?\s*up)\s+at\s+'
            r'(\d{1,2}:\d{2}\s*(?:a\.?m\.?|p\.?m\.?)?(?:\s*[AaPp][Mm])?(?:\s*(?:ET|EST|EDT|Eastern))?)',
            clean_answer, re.IGNORECASE
        )
        if m_end_prose:
    ...
    if m_bcast:
        ...
        # Prose fallback: "aired on USA Network, The U, and FOX 5 New York"
        m_bcast_prose = re.search(
            r'(?:aired?\s+on|broadcast\s+on|televised\s+on|shown\s+on|available\s+on)\s+(.+?)(?:\.|$)',
            clean_answer, re.IGNORECASE
        )
    ...
    # Prose fallback: scan full text for overtime/delay keywords if Notes label not found
    if not notes_text:
        notes_text = clean_answer

...
def _merge_lookup(base: dict, extra: dict):
    ...
    for key in ["actual_start","actual_end","video_duration","national_channel","local_home","local_away","streaming","status_hint","notes"]:
...
def multi_source_lookup(event: dict, api_key: str, run_type: str) -> dict:
    ...
    # 9. Google Game Times (SerpApi — dedicated start/end/notes queries, highest priority for times)
    if api_key:
        ggt = google_game_times_lookup(event, api_key)
        if ggt.get("actual_start"):
            combined["actual_start"] = ggt["actual_start"]
        if ggt.get("actual_end"):
            combined["actual_end"] = ggt["actual_end"]
        if ggt.get("notes"):
            combined["notes"] = ggt["notes"]
        if ggt.get("details"):
            combined["details"] = ggt["details"]
        combined["sources_used"].append("Google Times")
...
# ─────────────────────────────────────────────
# GOOGLE GAME TIMES LOOKUP (SerpApi)
# ─────────────────────────────────────────────

def google_game_times_lookup(event: dict, api_key: str) -> dict:
    """Use SerpApi to find actual game start time, end time, and status notes.
    Queries: 'what time did [event] start/end [date]'
    Extracts times from Google's sports knowledge panel and snippet text.
    Returns: actual_start, actual_end, notes (overtime/delay/postponed text).
    """
    result = {"actual_start": "", "actual_end": "", "notes": "", "details": ""}
    if not api_key:
        return result
    ...
    # Time patterns: strict AM/PM + timezone
    TIME_PAT = re.compile(
        r'\b(\d{1,2}:\d{2}\s*(?:AM|PM|a\.m\.|p\.m\.)\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT)?)\b',
        re.IGNORECASE
    )
    # End-time anchor phrases (ordered by specificity)
    END_PATS = [
        re.compile(r'ended?\s+(?:at\s+)?(?:approximately\s+)?(\\d{1,2}:\\d{2}\\s*(?:AM|PM|a\\.m\\.|p\\.m\\.)\\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT|Eastern(?:\\s+Time)?|Central(?:\\s+Time)?|Mountain(?:\\s+Time)?|Pacific(?:\\s+Time)?|local(?:\\s+time)?)?)', re.IGNORECASE),
        re.compile(r'(?:final|concluded?|finished?|wrapped?|over|completed?)\s+(?:at\s+)?(\\d{1,2}:\\d{2}\\s*(?:AM|PM|a\\.m\\.|p\\.m\\.)\\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT|Eastern(?:\\s+Time)?|Central(?:\\s+Time)?|Mountain(?:\\s+Time)?|Pacific(?:\\s+Time)?|local(?:\\s+time)?)?)', re.IGNORECASE),
        re.compile(r'(\d{1,2}:\d{2}\s*(?:AM|PM|a\.m\.|p\.m\.)\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT)?)\s*(?:final|end|ended|ET\)?)', re.IGNORECASE),
        re.compile(r'end\s+time[:\s]+(?:~)?(\\d{1,2}:\\d{2}\\s*(?:AM|PM|a\\.m\\.|p\\.m\\.)\\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT|Eastern(?:\\s+Time)?|Central(?:\\s+Time)?|Mountain(?:\\s+Time)?|Pacific(?:\\s+Time)?|local(?:\\s+time)?)?)', re.IGNORECASE),
    ]
    # Start-time anchor phrases
    START_PATS = [
        re.compile(r'started?\s+(?:at\s+)?(\\d{1,2}:\\d{2}\\s*(?:AM|PM|a\\.m\\.|p\\.m\\.)\\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT|Eastern(?:\\s+Time)?|Central(?:\\s+Time)?|Mountain(?:\\s+Time)?|Pacific(?:\\s+Time)?|local(?:\\s+time)?)?)', re.IGNORECASE),
        re.compile(r'start\s+time[:\s]+(\\d{1,2}:\\d{2}\\s*(?:AM|PM|a\\.m\\.|p\\.m\\.)\\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT|Eastern(?:\\s+Time)?|Central(?:\\s+Time)?|Mountain(?:\\s+Time)?|Pacific(?:\\s+Time)?|local(?:\\s+time)?)?)', re.IGNORECASE),
        re.compile(r'tip-?off\s+(?:at\s+)?(\\d{1,2}:\\d{2}\\s*(?:AM|PM|a\\.m\\.|p\\.m\\.)\\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT|Eastern(?:\\s+Time)?|Central(?:\\s+Time)?|Mountain(?:\\s+Time)?|Pacific(?:\\s+Time)?|local(?:\\s+time)?)?)', re.IGNORECASE),
        re.compile(r'first\s+pitch\s+(?:at\s+)?(\\d{1,2}:\\d{2}\\s*(?:AM|PM|a\\.m\\.|p\\.m\\.)\\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT|Eastern(?:\\s+Time)?|Central(?:\\s+Time)?|Mountain(?:\\s+Time)?|Pacific(?:\\s+Time)?|local(?:\\s+time)?)?)', re.IGNORECASE),
        re.compile(r'kickoff\s+(?:at\s+)?(\\d{1,2}:\\d{2}\\s*(?:AM|PM|a\\.m\\.|p\\.m\\.)\\s*(?:ET|CT|MT|PT|EST|CST|MST|PST|EDT|CDT|MDT|PDT|Eastern(?:\\s+Time)?|Central(?:\\s+Time)?|Mountain(?:\\s+Time)?|Pacific(?:\\s+Time)?|local(?:\\s+time)?)?)', re.IGNORECASE),
    ]
    # Notes keywords to capture from snippet text
    NOTES_PATTERNS = [
        (re.compile(r'overtime|OT\b', re.IGNORECASE), "Overtime"),
        ...
        (re.compile(r'suspended', re.IGNORECASE), "Suspended"),
        (re.compile(r'extra\s*innings?', re.IGNORECASE), "Extra innings"),
        (re.compile(r'walk[-\s]?off', re.IGNORECASE), "Walk-off"),
        (re.compile(r'shootout', re.IGNORECASE), "Shootout"),
        (re.compile(r'penalty\s*kicks?|PKs?\b', re.IGNORECASE), "Penalty kicks"),
    ]

    def _query_google(q: str) -> dict:
        try:
            resp = requests.get("https://serpapi.com/search", params={
                "q": q, "api_key": api_key, "engine": "google", "num": 5,
            }, timeout=15)
            return resp.json()
        except Exception:
            return {}

    def _extract_time_from_data(data: dict, prefer_end: bool = False) -> tuple[str, str, list[str]]:
        """Return (start_time, end_time, notes_list) from a SerpApi response.
        Strategy: collect ALL time mentions across all result sections, then
        use first = start, last = end (games end later than they start).
        """
        all_times = []
        start_anchors = []
        end_anchors = []
        ...
        # Timezone offset map: how many hours to ADD to convert to ET
        TZ_OFFSETS = {
            "ET": 0, "EST": 0, "EDT": 0,
            "CT": 1, "CST": 1, "CDT": 1,
            "MT": 2, "MST": 2, "MDT": 2,
        ...
        def _to_et_minutes(hour: int, minute: int, is_pm: bool, tz: str) -> tuple[int, int]:
            """Convert any timezone to ET, return (hour_24, minute)."""
            h24 = hour % 12 + (12 if is_pm else 0)
            offset = TZ_OFFSETS.get(tz.upper(), 0)
            total = h24 * 60 + minute + offset * 60
            total %= 1440  # wrap at midnight
            return total // 60, total % 60

        def _normalize_tz(t_raw: str) -> str:
            """Normalize any timezone to ET, convert time math, clean P.M./A.M. formatting.
            
            Handles:
            - "11:00 a.m. CT" → "12:00 PM ET"
            - "1:00 p.m. CT"  → "2:00 PM ET"
            - "6:00 PM PDT"   → "9:00 PM ET"
            - "8:00 P.M. ET"  → "8:00 PM ET"
            - "12:00 p.m. ET" → "12:00 PM ET"
            
            If Google gives parenthesized ET equivalent like "(12:00 p.m. ET)",
            that is preferred and extracted directly without conversion.
            """
            t = t_raw.strip()
            # Normalize P.M./A.M. with periods → PM/AM
            t = re.sub(r'p\.m\.?', 'PM', t, flags=re.IGNORECASE)
            t = re.sub(r'a\.m\.?', 'AM', t, flags=re.IGNORECASE)
            t = t.strip()
            # Parse: HH:MM AM/PM TZ
            # Normalize spelled-out timezone words to abbreviations
            t = re.sub(r'Eastern\s+Time', 'ET', t, flags=re.IGNORECASE)
            t = re.sub(r'Central\s+Time', 'CT', t, flags=re.IGNORECASE)
            t = re.sub(r'Mountain\s+Time', 'MT', t, flags=re.IGNORECASE)
            t = re.sub(r'Pacific\s+Time', 'PT', t, flags=re.IGNORECASE)
            t = re.sub(r'\bEastern\b', 'ET', t, flags=re.IGNORECASE)
            t = re.sub(r'\bCentral\b', 'CT', t, flags=re.IGNORECASE)
            t = re.sub(r'\bMountain\b', 'MT', t, flags=re.IGNORECASE)
            t = re.sub(r'\bPacific\b', 'PT', t, flags=re.IGNORECASE)
            t = re.sub(r'\blocal(?:\s+time)?\b', '', t, flags=re.IGNORECASE).strip()
            m = re.match(r'(\d{1,2}):(\d{2})\s*(AM|PM)\s*(ET|EST|EDT|CT|CST|CDT|MT|MST|MDT|PT|PST|PDT)?', t, re.IGNORECASE)
            if not m:
                return t.upper() + " ET"
            hour = int(m.group(1))
            ...
            is_pm = (ampm == "PM")
            if tz in ("ET", "EST", "EDT"):
                # Already ET — just clean up
                return f"{hour}:{minute:02d} {ampm} ET"
            # Convert to ET
            h_et, min_et = _to_et_minutes(hour, minute, is_pm, tz)
            ampm_et = "PM" if h_et >= 12 else "AM"
            h12 = h_et % 12 or 12
            return f"{h12}:{min_et:02d} {ampm_et} ET"

        def _add_time(t_raw: str):
            t = _normalize_tz(t_raw)
            if t not in all_times:
                all_times.append(t)

        # 1. Sports widget (most reliable for start time)
        sr = data.get("sports_results", {})
        if sr:
            spotlight = sr.get("game_spotlight", {})
            if isinstance(spotlight, dict):
                for field in ["match_time", "time", "date"]:
                    tv = spotlight.get(field, "")
                    if tv:
                        cleaned = _parse_game_time(tv)
                        if cleaned:
                            _add_time(cleaned)
                            break
            status_raw = (sr.get("status", "") or "").strip()
            if status_raw:
                for pat, label in NOTES_PATTERNS:
                    if pat.search(status_raw) and label not in notes:
                        notes.append(label)
                if len(status_raw) < 80 and status_raw not in notes:
                    notes.append(status_raw)

        # 2. Answer box (Google AI Overview / featured snippet — most reliable)
        ab = data.get("answer_box", {})
        if isinstance(ab, dict):
            for field in ["answer", "result", "snippet", "description"]:
                val = ab.get(field, "")
                if not val:
                    continue
                for pat, label in NOTES_PATTERNS:
                    if pat.search(val) and label not in notes:
                        notes.append(label)
                # PRIORITY 0: Look for parenthesized ET equivalent
                # e.g. "11:00 a.m. CT (9:00 a.m. PT / 12:00 p.m. ET)"
                # The ET value in parens is the most reliable — extract it directly
                # Extract Eastern time from parenthetical equivalents
                # Handles: "(9:00 PM Eastern / 8:00 PM Central)"
                #          "(9:00 PM ET)"
                #          "(12:00 p.m. ET)"
                ET_PAREN_PAT = re.compile(
                    r'(\d{1,2}:\d{2}\s*(?:AM|PM|a\.m\.|p\.m\.))'
                    r'\s*(?:ET|EST|EDT|Eastern(?:\s+Time)?)',
                    re.IGNORECASE
                )
                et_parens = ET_PAREN_PAT.findall(val)
                et_times_norm = []
                for ep_val in et_parens:
                    # Normalize and add as ET
                    ep_clean = re.sub(r'p\.m\.?', 'PM', ep_val, flags=re.IGNORECASE)
                    ep_clean = re.sub(r'a\.m\.?', 'AM', ep_clean, flags=re.IGNORECASE)
                    ep_clean = ep_clean.strip() + " ET"
                    _add_time(ep_clean)
                    if ep_clean not in et_times_norm:
                        et_times_norm.append(ep_clean)
                # If Eastern values found in parens, assign first=start, last=end (highest priority)
                # e.g. "started at 6:00 PM local Pacific Time (9:00 PM Eastern / 8:00 PM Central)
                #       and ended at approximately 8:07 PM local time (11:07 PM Eastern / ...)"
                if et_times_norm:
                    if et_times_norm[0] not in start_anchors:
                        start_anchors.append(et_times_norm[0])
                    if len(et_times_norm) > 1 and et_times_norm[-1] not in end_anchors:
                        end_anchors.append(et_times_norm[-1])
                # Try anchor patterns on answer box text ONLY if no ET-in-parens found
                for sp in START_PATS:
                    m = sp.search(val)
                    if m:
                        t = _normalize_tz(m.group(1))
                        if not start_anchors and t not in start_anchors:
                            start_anchors.append(t)
                        if not start_anchors and t not in all_times:
                            all_times.insert(0, t)
                for ep in END_PATS:
                    m = ep.search(val)
                    if m:
                        t = _normalize_tz(m.group(1))
                        if not end_anchors and t not in end_anchors:
                            end_anchors.append(t)
                        if not end_anchors and t not in all_times:
                            all_times.append(t)
                # Fallback: any time mentions in answer box
                for t in TIME_PAT.findall(val):
                    _add_time(t)

        # 2a. Also process ai_overview.text_blocks for time extraction
        # SerpApi returns AI Overview as ai_overview.text_blocks[].snippet
        ai_ov = data.get("ai_overview", {})
        if isinstance(ai_ov, dict):
            for block in ai_ov.get("text_blocks", []):
                val = block.get("snippet", "") or block.get("text", "")
                if not val:
                    continue
                for pat, label in NOTES_PATTERNS:
                    if pat.search(val) and label not in notes:
                        notes.append(label)
                ET_PAREN_PAT2 = re.compile(
                    r'(\d{1,2}:\d{2}\s*(?:AM|PM|a\.m\.|p\.m\.))' 
                    r'\s*(?:ET|EST|EDT|Eastern(?:\s+Time)?)',
                    re.IGNORECASE
                )
                et_parens2 = ET_PAREN_PAT2.findall(val)
                et_times_norm2 = []
                for ep_val in et_parens2:
                    ep_clean = re.sub(r'p\.m\.?', 'PM', ep_val, flags=re.IGNORECASE)
                    ep_clean = re.sub(r'a\.m\.?', 'AM', ep_clean, flags=re.IGNORECASE)
                    ep_clean = ep_clean.strip() + " ET"
                    _add_time(ep_clean)
                    if ep_clean not in et_times_norm2:
                        et_times_norm2.append(ep_clean)
                if et_times_norm2:
                    if et_times_norm2[0] not in start_anchors:
                        start_anchors.append(et_times_norm2[0])
                    if len(et_times_norm2) > 1 and et_times_norm2[-1] not in end_anchors:
                        end_anchors.append(et_times_norm2[-1])
                for sp in START_PATS:
                    m = sp.search(val)
                    if m:
                        t = _normalize_tz(m.group(1))
                ...
                for t in TIME_PAT.findall(val):
                    _add_time(t)
        # 2b. Capture raw AI Overview / answer box text for DETAILS column
        # ONLY use answer_box or ai_overview — never organic snippets (those are schedule pages)
        # The text must look like a game-time sentence to be useful.
        GAME_TIME_KEYWORDS = re.compile(
            r'\b(started|ended|start(?:ed)?\s+at|end(?:ed)?\s+at|game\s+on|game\s+at|'
            r'tip.?off|kick.?off|first\s+pitch|began|concluded|finished|lasted)\b',
            re.IGNORECASE
        )
        raw_detail = ""
        # Check SerpApi answer_box (featured snippet / AI Overview)
        if isinstance(ab, dict):
            for field in ["answer", "result", "snippet", "description", "title"]:
                val = ab.get(field, "")
                if val and len(val) > 30 and GAME_TIME_KEYWORDS.search(val):
                    raw_detail = val.strip()
                    break
        # Check SerpApi ai_overview key — Google AI Overview
        # SerpApi returns: ai_overview.text_blocks[].snippet  (paragraph type)
        # Also check direct fields for older API versions
        if not raw_detail:
            ai_ov = data.get("ai_overview", {})
            if isinstance(ai_ov, dict):
                # PRIMARY: iterate text_blocks (the actual SerpApi AI Overview format)
                for block in ai_ov.get("text_blocks", []):
                    snip = block.get("snippet", "") or block.get("text", "")
                    if snip and len(snip) > 30 and GAME_TIME_KEYWORDS.search(snip):
                        raw_detail = snip.strip()
                        break
                # FALLBACK: direct fields (older format)
                if not raw_detail:
                    for field in ["answer", "snippet", "text", "description"]:
                        val = ai_ov.get(field, "")
                        if val and len(val) > 30 and GAME_TIME_KEYWORDS.search(val):
                            raw_detail = val.strip()
                            break
            elif isinstance(ai_ov, str) and len(ai_ov) > 30 and GAME_TIME_KEYWORDS.search(ai_ov):
                raw_detail = ai_ov.strip()
        # Check organic snippets ONLY if they look like a game-time sentence
        # (not schedule pages — those say "Explore the complete 2026 schedule...")
        if not raw_detail:
            for organic in data.get("organic_results", [])[:3]:
                snip = organic.get("snippet", "") or ""
                if snip and len(snip) > 40 and GAME_TIME_KEYWORDS.search(snip):
                    raw_detail = snip.strip()
                    break
        if raw_detail and not result.get("details"):
            result["details"] = raw_detail
            result["details"] = raw_detail

        # 3. Knowledge graph
        kg = data.get("knowledge_graph", {})
        if isinstance(kg, dict):
            for field in ["description", "time", "date"]:
                val = kg.get(field, "")
                if val:
                    for t in TIME_PAT.findall(str(val)):
                        _add_time(t)

        # 4. Organic snippets — collect all times + notes
        for organic in data.get("organic_results", []):
            snippet = (organic.get("snippet", "") or "") + " " + (organic.get("title", "") or "")
            if not snippet.strip():
                continue
            for pat, label in NOTES_PATTERNS:
                if pat.search(snippet) and label not in notes:
                    notes.append(label)
            # Start-time anchor phrases (highest confidence for start time)
            for sp in START_PATS:
                m = sp.search(snippet)
                if m:
                    t = _normalize_tz(m.group(1))
                    if t not in start_anchors:
                        start_anchors.append(t)
                    if t not in all_times:
                        all_times.insert(0, t)  # start anchors go to front
            # End-time anchor phrases (highest confidence for end time)
            for ep in END_PATS:
                m = ep.search(snippet)
                if m:
                    t = _normalize_tz(m.group(1))
                    if t not in end_anchors:
                        end_anchors.append(t)
                    if t not in all_times:
                        all_times.append(t)  # end anchors go to back
            # All other times in snippet — only add if anchors not yet found
            # (avoids polluting all_times with scheduled/other-game times)
            if not start_anchors and not end_anchors:
                for t in TIME_PAT.findall(snippet):
                    _add_time(t)

        # Strict: only use anchor-matched times.
        # Never fall back to unanchored "first/last" heuristic — those times
        # come from scheduled listings or other games and produce wrong results.
        start = start_anchors[0] if start_anchors else ""
        end   = end_anchors[-1]  if end_anchors   else ""

        # Special case: prefer_end=True (dedicated end-time query) — if only
        # one anchor found and it looks like an end time, use it as end.
        if prefer_end and not end and start:
            end   = start
            start = ""

        return start, end, notes, raw_detail

    # Single combined query: "what time did the [team] [date] game start and end"
    # This matches the Google AI Overview format which returns both times in one response.
    # Use the home/away team name (shorter, more specific) rather than full event name.
    team_hint = (event.get("home_team") or event.get("away_team") or name).split("@")[0].strip()
    q = f"what time did the {team_hint} {date_str} game start and end"
    data = _query_google(q)
    s1, e1, notes1, d1 = _extract_time_from_data(data, prefer_end=False)
    if s1:
        result["actual_start"] = s1
    if e1:
        result["actual_end"] = e1
    if d1 and not result.get("details"):
        result["details"] = d1

    # Fallback: if end time still not found, try a dedicated end-time query
    if not result["actual_end"] and result["actual_start"]:
        q_end = f"what time did the {team_hint} {date_str} game end"
        data_end = _query_google(q_end)
        _, e2, notes2, d2 = _extract_time_from_data(data_end, prefer_end=True)
        if e2:
            result["actual_end"] = e2
        if d2 and not result.get("details"):
            result["details"] = d2
        notes1 = list(dict.fromkeys(notes1 + notes2))

    result["notes"] = "; ".join(notes1) if notes1 else ""
    return result
...
def sportsmediawatch_lookup(event: dict, run_type: str) -> dict:
    ...
    """
    # League → keywords that must appear near a matched line to confirm it is the right sport.
    # This prevents cross-sport matches (e.g., WNBA "Minnesota" matching MLB "Minnesota Twins").
    LEAGUE_CONTEXT_KEYWORDS = {
        "MLB":    ["mlb","baseball"],
        "NFL":    ["nfl","football"],
        "NBA":    ["nba","basketball"],
        "WNBA":   ["wnba","women's basketball","women's nba"],
        "NHL":    ["nhl","hockey"],
        "MLS":    ["mls","soccer","major league soccer"],
        "NCAAF":  ["ncaaf","college football","cfb"],
        "NCAAB":  ["ncaab","college basketball","cbb"],
        "NASCAR": ["nascar","cup series","xfinity series"],
        "NHRA":   ["nhra","drag racing","top fuel","funny car"],
        "PBR":    ["pbr","bull riding"],
        "FISHING":["fishing","bass","angling"],
        "BOXING": ["boxing","bout","fight"],
        "TENNIS": ["tennis","atp","wta","open"],
        "GOLF":   ["golf","pga","lpga"],
        "INDYCAR":["indycar","indy 500","indy car"],
    ...
    result = {"actual_start":"","actual_end":"","video_duration":"","national_channel":"",
              "local_home":"","local_away":"","streaming":"","status_hint":""}
    league = event.get("league","").upper()
    context_kws = LEAGUE_CONTEXT_KEYWORDS.get(league, [])
    try:
        resp = requests.get(
    ...
    for i, line in enumerate(lines):
        ...
        if not any(t in line_lower for t in all_terms):
            continue
        # League-context guard: verify surrounding window contains a sport keyword.
        if context_kws:
            window_context = lines[max(0, i-5):i+6]
            window_text = " ".join(window_context).lower()
            if not any(kw in window_text for kw in context_kws):
                continue
...
def run_verification(events: list[dict], run_type: str, api_key: str, run_name: str) -> pd.DataFrame:
    ...
    for col_def in [("video_duration","TEXT"), ("sources_used","TEXT"), ("notes","TEXT"), ("details","TEXT")]:
        try:
    ...
    for i, event in enumerate(events):
        ...
        c.execute("""INSERT INTO verification_results
            (run_id,league,event_name,event_date,sched_start,sched_channel,
             ...
             streaming,verdict,notes,search_url,sources_used,details)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (row["run_id"],row["league"],row["event_name"],row["event_date"],
             ...
             row["streaming"],row["verdict"],row.get("notes",""),row["search_url"],row["sources_used"],
             row.get("details","")))
        conn.commit()
...
def render_results_table(df: pd.DataFrame):
    ...
    for _, row in df.iterrows():
        ...
        details_disp  = (details_raw[:80] + "…") if len(details_raw) > 80 else (details_raw or "—")
        details_title = details_raw.replace('"', '&quot;')
        ...
        <tr style="background:{bg}; border-bottom:1px solid #30363d;">
            ...
            <td style="padding:6px 10px; color:#f0b429; font-size:11px; max-width:180px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="{notes_title}">{notes_disp}</td>
            <td style="padding:6px 10px; color:#8b949e; font-size:11px; max-width:260px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="{details_title}">{details_disp}</td>
    ...
    with tab_dash:
        ...
        if "results_df" in st.session_state:
            ...
            export_cols = ["league","event_name","event_date","sched_start","sched_channel",
                           ...
                           "local_home","local_away","streaming","notes","details","verdict","sources_used"]
            ...
            col_labels = {
                ...
                "local_home":"Local Home","local_away":"Local Away","streaming":"Streaming",

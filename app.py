...
def google_game_times_lookup(event: dict, api_key: str) -> dict:
    ...
    if m_bcast:
        ...
        m_bcast_prose = re.search(
            r'(?:aired?\s+on|broadcast\s+on|televised\s+on|shown\s+on)\s+([^.;]+?)(?:\.|;|,\s*(?:did\s+not|had\s+(?:no|a\s+typical|an?\s+)|it\s+was|and\s+it\s+did|with\s+no|which\s+did|there\s+was|no\s+overtime|no\s+weather|no\s+rain|no\s+delay|a\s+typical|the\s+game\s+did))',
            clean_answer, re.IGNORECASE
        )
        # Additional fallbacks for other Perplexity phrasings
        if not m_bcast_prose:
            m_bcast_prose = re.search(
                r'(?:listed\s+on|available\s+on|can\s+be\s+seen\s+on|airing\s+on|listed\s+as\s+airing\s+on|broadcast\s+channels?\s*(?:were|are|:)?)\s+([^.;\n]+?)(?:\.|;|\n)',
                clean_answer, re.IGNORECASE
            )
        if not m_bcast_prose:
            # Try "It aired on X" with bullet list format: "- NBA TV\n- CW26\n- FOX12+"
            m_bcast_prose = re.search(
                r'(?:aired?\s+on|broadcast\s+on|available\s+on)[^:\n]*:\s*\n((?:\s*[-*•]\s*[^\n]+\n?)+)',
                clean_answer, re.IGNORECASE
            )
        bcast_raw = m_bcast_prose.group(1).strip() if m_bcast_prose else ""
        # If bullet list matched, flatten bullet lines into comma-separated string
        if bcast_raw and re.search(r'^\s*[-*•]', bcast_raw, re.MULTILINE):
            lines = re.findall(r'[-*•]\s*(.+)', bcast_raw)
    ...
    if scan_text:
        ...
        NEGATION_RE = re.compile(
            ...
            r'absence of|without any|no\s+(?:overtime|delay|weather|rain|postpone))\b',
            re.IGNORECASE
        )
        found = []
